...
class Configuration:
    ...
    def hotkey_pressed(self):
        if self.session.value == 2:
            if self.__class__.race_mode >= 6: # if tires goto gaps
                self.__class__.race_mode = 1 
            elif self.__class__.race_mode == 2: # if intervals go to compact
                self.__class__.race_mode = 4 
            else:
                self.__class__.race_mode += 1
            ac.setValue(self.spin_race_mode, self.__class__.race_mode)
        else:
            if self.__class__.qual_mode >= 2: # gaps > times > sectors
                self.__class__.qual_mode = 0
            else:
